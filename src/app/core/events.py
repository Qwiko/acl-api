...
from app.models import DynamicPolicy, NetworkAddress, Policy, PolicyTerm, ServiceEntry, Target
...
async def handle_network_address(session: AsyncSession, obj: NetworkAddress):
    ...
    for nested_ref_network in networks:
        handle_network_address(session, nested_ref_network)
...
async def handle_service_entry(session: AsyncSession, obj: ServiceEntry):
    ...
    for nested_ref_service in services:
        handle_service_entry(session, nested_ref_service)
        ...
        async def internal_func(obj_data):
            ...
            for obj in obj_data:
                if isinstance(obj, NetworkAddress):
                    await handle_network_address(async_session, obj)
                elif isinstance(obj, ServiceEntry):
                    await handle_service_entry(async_session, obj)
