...
def test_post_policy_term(db: Session, client: TestClient) -> None:
    nested_policy_1 = generators.create_policy(db)
    nested_policy_2 = generators.create_policy(db)
    policy = generators.create_policy(db)

    # Test creating a term
    response = client.post(
        f"/api/v1/policies/{policy.id}/terms",
        json={
            "name": fake.name(),
            "action": "accept",
            "logging": True,
            ...
            # "nested_policy_id"     : nested_policy_1.id,
        },
    )
    assert response.status_code == status.HTTP_201_CREATED

    # Test creating a term with a nested policy
    response = client.post(
        f"/api/v1/policies/{policy.id}/terms",
        json={
            "name": fake.name(),
    ...
    # Test adding nested_policy_2
    response = client.post(
        f"/api/v1/policies/{policy.id}/terms",
        json={
            "name": fake.name(),
            "enabled": True,
            "nested_policy_id": nested_policy_2.id,
        },
    )
    assert response.status_code == status.HTTP_201_CREATED

    # Test adding nested_policy_2 again
    response = client.post(
        f"/api/v1/policies/{policy.id}/terms",
        json={
            "name": fake.name(),
    ...
    # Test deleting the nested policy
    response = client.delete(
        f"/api/v1/policies/{nested_policy_1.id}",
    )
    assert response.status_code == status.HTTP_403_FORBIDDEN

    # Test creating a term with a non-existent nested policy
    response = client.post(
        f"/api/v1/policies/{policy.id}/terms",
        json={
            "name": fake.name(),
            "enabled": True,
            "nested_policy_id": 99999999,
        },
    )
    assert response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY

    # Test creating a term with too much data
    response = client.post(
        f"/api/v1/policies/{policy.id}/terms",
        json={
            "name": fake.name(),
    ...
    # Test creating a term with a non-existent policy
    response = client.post(
        f"/api/v1/policies/{99999999}/terms",
        json={
            "name": fake.name(),
            "enabled": False,
            "nested_policy_id": 99999999,
